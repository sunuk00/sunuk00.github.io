...
공이 센서 위 특정 지점을 누르면, 그 지점을 기준으로 저항이 두 구간으로 나뉜다. \[공,오른쪽\] 구간의 저항과 \[왼쪽,공\] 구간의 저항이다. 공이 오른쪽에 가까울수록 \[공,오른쪽\] 구간은 짧아져 저항이 작아지고, 반대로 \[왼쪽,공\] 구간은 길어져 저항이 커진다. 이렇게 공의 위치에 따라 두 구간의 저항 비율이 실시간으로 바뀌고, 이 비율이 그 지점(OUT 핀)에서 출력되는 전압값을 결정한다. 즉 저항 비율의 변화가 원인이고, 전압의 변화는 그 결과다.
