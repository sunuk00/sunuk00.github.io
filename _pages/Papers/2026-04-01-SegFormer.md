...
**A3.** 논문에서도 초기 레이어(Stage-1)에서는 해상도가 너무 커서 계산량이 무지막지하므로 Reduction Ratio $R$을 64로 크게 잡아 계산량을 대폭 깎고, 해상도가 작아지는 후기 레이어(Stage-4)로 갈수록 $R=1$로 설정해 정보 손실이 없도록 유연하게 설계함.
