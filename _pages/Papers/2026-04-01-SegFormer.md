...
구조는 간단하다. 여러 scale의 feature를 같은 크기와 같은 채널로 맞춘 뒤, concat하고 MLP로 융합한다. 

<figure style="margin: 0; text-align: center;">
    <img src="/assets/img/posts/Papers/segformer007.jpg" width="800" height="300" />
    <figcaption></figcaption>
</figure> 
...
Stage-4에서 얻은 feature는 non-local 정보를 담고 있지만, local 정보는 담지 못한다. 따라서 Stage-1~3에서 얻은 feature와 Stage-4에서 얻은 feature를 통합해야 한다. SegFormer의 MLP decoder는 이 두 feature를 통합하여 전역적이고 국소적인 정보를 동시에 얻는다

사실 SegFormer의 Encoder가 정말 강력하다. 좋은 Encoder가 있으면 복잡한 Decoder가 필요없음을 증명한 듯하다.

<figure style="margin: 0; text-align: center;">
    <img src="/assets/img/posts/Papers/segformer008.png" width="550" height="300" />
    <figcaption></figcaption>
</figure> 
...
SegFormer는 CNN기반인 DeepLabV3+와 비교했을 때, 훨씬 더 넓은 ERF를 가진다. 즉, SegFormer의 Encoder가 훨씬 더 많은 contextual information을 담고 있음을 알 수 있다.


### Experiments
개인적으로 젤 중요하다고 생각한 실험은 이 세 개다.
<figure style="margin: 0; text-align: center;">
    <img src="/assets/img/posts/Papers/segformer010.png" width="720" height="250" />
    <figcaption>파라미터에 따른 모델 성능</figcaption>
    <img src="/assets/img/posts/Papers/segformer009.png" width="720" height="280" />
    <figcaption></figcaption>
</figure> 


## Questions
**Q1.** SegFormer는 positional encoding을 하지 않지만, 기존 ViT는 학습 이미지와 추론 이미지의 resolution이 다를 때 interpolating positional codes가 필요했다. 왜 positional encoding이 정확도를 떨어트리지?

**A1.** 기존 ViT의 고정된 Positional Encoding(PE)은 특정 학습 해상도의 공간적 위치를 1:1로 기억하고 있기 때문에, 추론 시 해상도가 달라지면 입력 크기에 맞게 bilinear 보간법 등으로 억지로 늘려야 한다. 이 과정에서 고차원 벡터의 기하학적 정보가 왜곡되어 모델이 학습했던 절대적 위치 관계가 붕괴되고, 결국 픽셀 단위의 정밀도가 중요한 세그멘테이션 성능 하락으로 이어진다.

---

**Q2.** Efficient Self-Attention에서 토큰의 배열을 N X C를 N/R X C*R로 바꾸면, 위치 관계가 깨지지 않나? 애초에 attention은 위치 관계를 고려하지 않아서 상관없나?

**A2.** 위치 관계가 깨지는 것이 맞지만, 공간적으로 인접한 토큰들끼리 묶이고, Linear Layer를 거치면서 C*R → C로 줄일 때, 이 집합의 대표 특징을 뽑아낸다.

---

**Q3.** High resolution을 self-attention에 넣으면 계산량이 너무 많아져서, K값을 줄여서 계산량을 줄였는데, linear layer를 거치면서 K값이 줄어들면, 결국 attention을 계산할 때, K값이 줄어든 만큼 정보가 손실되지 않을까? 계산량과 정확도 간의 trade-off가 존재하지 않을까?

**A3.** trade-off가 존재하고, 논문에서도 초기 레이어(Stage-1)에서는 해상도가 너무 커서 계산량이 무지막지하므로 Reduction Ratio $R$을 64로 크게 잡아 계산량을 대폭 깎고, 해상도가 작아지는 후기 레이어(Stage-4)로 갈수록 $R=1$로 설정해 정보 손실이 없도록 유연하게 설계했다.

---

**Q4.** 왜 하필 Q, K, V 중에서 K를 줄이는 걸까? Q나 V를 줄이면 안되는 이유가 있을까?

**A4.** Q를 N/R x C로 줄이게 되면,  Softmax($QK^T$)는 N/R x N이 된다. 이어서 V와 곱하면 N/R x C가 된다. 즉, 최종 output의 shape이 N/R x C가 되어버린다. 하지만 우리가 원하는 shape은 N x C이다. 따라서 Q를 줄이면 안된다.  사실 V는 이미 줄이고 있다. Softmax($QK^T$)는 N x N/R이 되고, V는 N/R x C이므로, 최종 output은  Softmax$(QK^T)V : N \times C$가 된다.
