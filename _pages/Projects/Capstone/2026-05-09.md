---
title: "Cross-Entropy Loss의 한계 & Focal Loss 추가"
tags:
    - python
    - pytorch
    - capstone
...
경로가 끊긴 이유는 아래 이미지처럼 경로 중간에 그려져 있는 '기호'를 '배경'으로 예측을 하기 때문이 아닐까 싶다.

<div style="display: flex; gap: 20px; justify-content: center;">
    <figure style="margin: 0; text-align: center;">
        <img src="../../../assets/img/posts/Capstone/016_0509.png" width="300" height="300"/>
        <figcaption>기호가 '경로'를 가리는 경우</figcaption>
    </figure>
</div>
...
<div style="display: flex; gap: 20px; justify-content: center;">
    ...
    <figure style="margin: 0; text-align: center;">
        <img src="../../../assets/img/posts/Capstone/017_0509.png" width="400" height="300" />
        <figcaption>모델이 예측한 경로 마스크</figcaption>
    </figure>
</div>
<br>
...
## BCE Loss + Focal Loss
BCE Loss와 Focal Loss를 함께 사용하여 모델을 학습시키는 방법도 있다. 이 경우, 최종 손실 함수는 다음과 같이 정의된다:

$$L = \kappa L_{BCE} + (1 - \kappa) L_{Focal}$$

$\kappa$는 0.5, Focal의 $\gamma$는 2, $\alpha$는 0.75로 설정했을 때의 결과는 다음과 같다:
...
<div style="display: flex; gap: 20px; justify-content: center;">
    ...
    <figure style="margin: 0; text-align: center;">
        <img src="../../../assets/img/posts/Capstone/018_0509.png" width="400" height="300" />
        <figcaption>모델이 예측한 경로 마스크</figcaption>
    </figure>
</div>
<br>
...
## 결론
Focal Loss는 Easy Negative에 대한 패널티를 줄이고, Hard Negative에 대한 패널티를 증가시키는 방식으로 클래스 불균형 문제를 완화하는 손실 함수이다. Focal Loss를 사용하여 모델을 학습시켜보았지만, 성능이 엄청나게 좋아지지는 않았고, 어떤 경우에는 오히려 더 안좋아진 것도 있다. 더 많은 실험을 돌려보던가, 아니면 다른 방법을 시도해봐야겠다.
