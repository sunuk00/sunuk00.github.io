...
## 개요
지금까지는 U-Net 모델을 활용하여 마라톤 경로를 segment하고자 했다. 그러나 U-Net은 CNN 기반 인코더의 특성상 receptive field가 제한적이어서, 이미지 전체에 걸쳐 이어지는 경로의 전체적인 맥락을 파악하는 데 한계가 있었다.
...
위의 그림은 SegFormer의 구조를 보여준다. SegFormer는 MiT(Mix Transformer) 인코더와 MLP 디코더로 구성되어 있다. MiT 인코더는 hierarchical 구조를 가지며, 다양한 해상도의 feature map을 생성한다. MLP 디코더는 이러한 feature map을 활용하여 최종 segmentation 결과를 생성한다.
...
(그리고 데이터가 너무 부족하지 않을까.. 라는 생각도 든다.)
